...
public class DoItYourselfModel implements IModelFunctions {
    ...
    public String createInstant(TimestampEntity instant) {
        ...
        // This verification takes too much time
...
//		}
    ...
    public String createObs(String value, String paramURI, String instantURI) {
        ...
        String uri = model.createInstance("Obs" + value + paramURI + instantURI, type);

        // Gets the timestamp of the given Instant
        String timestamp = getInstantTimestamp(instantURI);

//        System.out.println("Create Obs" + value + paramURI + timestamp);

        // Gets the sensor that did the observation and associates them
        String sensor = this.model.whichSensorDidIt(timestamp, paramURI);

        if (sensor != null) this.model.addObservationToSensor(uri, sensor);
		// If we did not find the sensor, we do not add the observation to the sensor
		else System.out.println("Did not find which sensor did the observation, cannot add observation to sensor");
